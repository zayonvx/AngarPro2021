...
const CalcPage003 = ({ ...props }: Props): JSX.Element => {
  const { gatesType, doorsType, gatesCount, doorsCount, windowsRows } = props;
  ...
  const handlerDropDownList = (evt: SyntheticEvent) => {
    const e = evt.currentTarget as HTMLInputElement;
    const gatesId = CALC_GATES.types.find((it) => e.value === it.name).id;
    const doorsId = CALC_DOORS.types.find((it) => e.value === it.name).id;
    const windowsId = CALC_WINDOWS.find((it) => e.value === it.name).id;
    switch (e.id) {
      case 'dropGatesType':
        store.dispatch(buildingChangeGatesType(gatesId));
        store.dispatch(buildingChangeGatesHeight(0));
        store.dispatch(buildingChangeGatesWidth(0));
        break;
      case 'dropDoorsType':
      ...
      <ul className={styles.inputList} style={gatesType !== 0 ? {} : { display: 'none' }}>
        <li key="001">
          <InputRange
            ...
            id="inputWidth"
            legend={`Ширина ворот${store.getState().building.gatesWidth}`}
          />
        </li>
        <li key="002">
          <InputRange
